...
impl ControlState {
    ...
    fn browser_seed_page(&mut self, url: &str) {
        ...
        if self
            .browser
            .page_text
            .contains("chostty-browser-comprehensive-1")
            || url.contains("comprehensive-1")
        {
            ...
            .browser
            .page_text
            .contains("chostty-browser-comprehensive-2")
        ...
        } else if self.browser.page_text.contains("chostty-browser-p0")
            ...
            || self.browser.page_text.contains("id=\"out\"")
        {
            ...
            || self.browser.page_text.contains("chostty-browser-extended")
        {
            self.browser.title = "chostty-browser-extended".to_string();
            self.browser
                .fields
                .insert("#name".to_string(), String::new());
            self.browser
            ...
            self.browser.title = "chostty-browser-extended-second".to_string();
            self.browser
                .dom_text
        ...
        if let Some(last_init) = self.browser.init_scripts.last() {
            if last_init.contains("__chosttyInitMarker") && last_init.contains("init-ok") {
...
fn fallback_ghostty_resources_dir() -> std::path::PathBuf {
    let root = std::env::temp_dir().join("chostty-ghostty-resources");
    ...
    if !terminfo_entry.exists() {
        let _ = std::fs::write(
            &terminfo_entry,
            "xterm-ghostty|chostty mock entry\n\tam,\n\tcols#80, lines#24,\n",
        );
    }
    root
}
...
fn run_terminal_command(
    ...
    shell.env("CHOSTTY_SURFACE_ID", encode_handle_id(surface_id));
...
fn debug_log_path() -> std::path::PathBuf {
    if let Ok(path) = std::env::var("CHOSTTY_DEBUG_LOG") {
        let trimmed = path.trim();
        if !trimmed.is_empty() {
            return std::path::PathBuf::from(trimmed);
        }
    }
    std::path::PathBuf::from("/tmp/chostty-debug.log")
}
        ...
        "browser.addscript" => {
            ...
            if script.contains("window.emitConsoleAndError") {
                state
                    .browser
                    .console
                    .push("chostty-console-entry".to_string());
                state.browser.errors.push("chostty-boom".to_string());
            }
            let value = state
                .browser
                .scripts
...
fn handle_debug_command(
    ...
    match method {
        ...
        "debug.panel_snapshot" => {
            ...
            let path = write_mock_png("chostty-panel", &label, surface_id)
        ...
        "debug.window.screenshot" => {
            ...
            let path = write_mock_png("chostty-window", &label, screenshot_id)
...
fn handle_command(
    ...
    match method {
        ...
        "browser.eval" => {
            ...
            } else if script.contains("__chosttyInitMarker") {
        ...
        "browser.screenshot" => Ok(json!({
            ...
            "format": "png",
            "bytes": 256,
            "path": "/tmp/chostty-browser-shot.png",
            "url": "file:///tmp/chostty-browser-shot.png",
...
mod tests {
    ...
    async fn dispatcher_handles_system_commands() {
        ...
        assert_eq!(identify_result["name"], "chostty-control");
    ...
    #[test]
    fn osc9_desktop_notifications_become_chostty_notifications() {
    ...
    #[test]
    fn osc9_control_byte_notifications_become_chostty_notifications() {
        let mut state = ControlState::default();
        let workspace_id = state.current_workspace_id;
        let surface_id = state.workspaces[0].windows[0].panes[0].surfaces[0].id;
    ...
    async fn dispatcher_handles_browser_file_url_navigation() {
        ...
        std::fs::write(
            &file_path,
            "<!doctype html><html><head><title>chostty file url load</title></head><body><h1>local HTML file loaded</h1></body></html>",
        )
