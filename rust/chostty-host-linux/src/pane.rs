...
const TAB_RENAME_ENTRY_CSS_CLASS: &str = "chostty-tab-rename-entry";
const TAB_RENAME_ENTRY_CSS_CLASSES: [&str; 2] = [HOST_ENTRY_CSS_CLASS, TAB_RENAME_ENTRY_CSS_CLASS];
const BROWSER_URL_ENTRY_CSS_CLASS: &str = "chostty-browser-url-entry";
const BROWSER_URL_ENTRY_CSS_CLASSES: [&str; 2] =
    [HOST_ENTRY_CSS_CLASS, BROWSER_URL_ENTRY_CSS_CLASS];
const BROWSER_SEARCH_ENTRY_CSS_CLASS: &str = "chostty-browser-search-entry";
...
.chostty-pane-content {
    background-color: transparent;
    border: none;
    box-shadow: none;
    outline: none;
}
.chostty-terminal-surface,
.chostty-terminal-glarea {
    background-color: transparent;
    border: none;
    box-shadow: none;
    outline: none;
...
.chostty-pane-focused .chostty-tab-active {
    color: @window_fg_color;
    background: alpha(@window_fg_color, 0.115);
    border-radius: 10px;
    border-right-color: transparent;
    box-shadow:
        inset 0 1px 0 alpha(@window_fg_color, 0.045),
        inset 0 0 0 1px alpha(@window_fg_color, 0.03);
}
.chostty-tab-close {
    background: none;
    border: none;
...
.chostty-pane-action:hover {
    background: alpha(@window_fg_color, 0.08);
    color: alpha(@window_fg_color, 0.8);
}
.chostty-pane-menu-btn {
    margin: 3px 0 3px 6px;
}
.chostty-pane-menu-btn > button {
    background: none;
    border: none;
    border-radius: 4px;
    padding: 4px 5px;
...
.chostty-pane-menu-btn > button:hover {
    background: alpha(@window_fg_color, 0.08);
    color: alpha(@window_fg_color, 0.8);
}
.chostty-pin-icon {
...
.chostty-browser-url-entry {
    min-height: 0;
    font-size: 12px;
}
.chostty-browser-search-entry {
    min-height: 0;
    font-size: 12px;
}
.chostty-tab-drop-indicator {
    background-color: @accent_bg_color;
    min-width: 2px;
    margin: 2px 0;
}
.chostty-tab-overlay:drop(active) {
    box-shadow: none;
}
.chostty-drop-preview {
    background: alpha(@accent_bg_color, 0.24);
    border: 1px solid alpha(@accent_bg_color, 0.65);
    border-radius: 10px;
}
.chostty-drop-preview-center {
...
pub fn create_pane(
    ...
    let header = gtk::Box::builder()
        ...
        .spacing(0)
        .build();
    header.add_css_class("chostty-pane-header");

    let tab_overlay = gtk::Overlay::new();
    tab_overlay.add_css_class("chostty-tab-overlay");
    ...
    let pane_menu_btn = gtk::MenuButton::builder()
        ...
        .build();
    pane_menu_btn.add_css_class("chostty-pane-menu-btn");

    let drop_indicator = gtk::Box::new(gtk::Orientation::Vertical, 0);
    drop_indicator.add_css_class("chostty-tab-drop-indicator");
    ...
    content_stack.add_css_class("chostty-pane-content");

    let content_overlay = gtk::Overlay::new();
    content_overlay.set_hexpand(true);
    content_overlay.set_vexpand(true);
    content_overlay.add_css_class("chostty-pane-content");
    ...
    unsafe {
        outer.set_data("chostty-pane-internals", internals);
    }

    // Track focus: add/remove .chostty-pane-focused on the outer box so CSS
    // can style the active tab differently in the focused vs unfocused pane.
    {
        ...
        outer.connect_state_flags_changed(move |widget, _| {
            ...
            if has_focus {
                header_for_focus.add_css_class("chostty-pane-focused");
            } else {
                header_for_focus.remove_css_class("chostty-pane-focused");
            }
...
pub fn cycle_tab_in_pane(pane_widget: &gtk::Widget, delta: i32) {
    ...
    let internals: Rc<PaneInternals> = unsafe {
        match outer.data::<Rc<PaneInternals>>("chostty-pane-internals") {
...
fn icon_button(icon_name: &str, tooltip: &str) -> gtk::Button {
    let btn = gtk::Button::builder()
        ...
        .has_frame(false)
        .build();
    btn.add_css_class("chostty-pane-action");
    btn
}
...
fn build_pane_menu_popover(
    ...
    let new_browser_btn = build_pane_menu_item("chostty-globe-symbolic", "New Browser Tab", None);
    let split_right_btn = build_pane_menu_item(
        "chostty-split-horizontal-symbolic",
        "Split Right",
        shortcuts.display_label_for_id(ShortcutId::SplitRight),
    );
    let split_down_btn = build_pane_menu_item(
        "chostty-split-vertical-symbolic",
        "Split Down",
...
fn find_pane_internals(pane_widget: &gtk::Widget) -> Option<Rc<PaneInternals>> {
    ...
    unsafe {
        outer
            .data::<Rc<PaneInternals>>("chostty-pane-internals")
...
pub fn is_pane_widget(widget: &gtk::Widget) -> bool {
    ...
    while let Some(current) = child {
        if current.has_css_class("chostty-pane-header") {
            return true;
        }
...
fn apply_pin_visuals(tab_button: &gtk::Box, pinned: bool) {
    if pinned {
        tab_button.add_css_class("chostty-tab-pinned");
    } else {
        tab_button.remove_css_class("chostty-tab-pinned");
    }
...
fn sync_tab_close_visibility(tab_button: &gtk::Box) {
    let pinned = tab_button.has_css_class("chostty-tab-pinned");
    let show_close = (tab_button.has_css_class("chostty-tab-hover")
        || tab_button.has_css_class("chostty-tab-active"))
        && !pinned;
...
fn new_tab_title_label(title: &str) -> gtk::Label {
    let label = gtk::Label::builder()
        ...
        .xalign(0.5)
        .build();
    label.add_css_class("chostty-tab-title");
...
fn build_tab_button_from_label(
    ...
    pin_icon.add_css_class("chostty-pin-icon");
    ...
    let close_btn = gtk::Button::builder()
        ...
        .has_frame(false)
        .build();
    close_btn.add_css_class("chostty-tab-close");
    ...
    tab_btn.add_css_class("chostty-tab");
    ...
    let hover = gtk::EventControllerMotion::new();
    {
        ...
        hover.connect_enter(move |_, _, _| {
            tab_btn.add_css_class("chostty-tab-hover");
    ...
    }
    {
        ...
        hover.connect_leave(move |_| {
            tab_btn.remove_css_class("chostty-tab-hover");
            sync_tab_close_visibility(&tab_btn);
        });
    }
...
fn clear_content_drop_zone(overlay: &gtk::Box) {
    overlay.remove_css_class("chostty-drop-preview");
    overlay.remove_css_class("chostty-drop-preview-center");
...
fn highlight_content_drop_zone(overlay: &gtk::Box, zone: ContentDropZone) {
    ...
    overlay.add_css_class("chostty-drop-preview");
    if zone == ContentDropZone::Center {
        overlay.add_css_class("chostty-drop-preview-center");
    }
...
fn sync_tab_separator_classes(tab_state: &Rc<RefCell<TabState>>) {
    ...
    for (idx, entry) in state.tabs.iter().enumerate() {
        ...
        if idx < last_idx && !next_is_active {
            entry.tab_button.add_css_class("chostty-tab-separator");
        } else {
            entry.tab_button.remove_css_class("chostty-tab-separator");
        }
    }
}
...
fn set_browser_targeting_enabled(content_stack: &gtk::Stack, enabled: bool) {
    ...
    while let Some(widget) = child {
        ...
        if !widget.has_css_class("chostty-browser") {
            continue;
        }
        let webview = widget
            .first_child()
...
fn activate_tab(
    ...
    for entry in &ts.tabs {
        if entry.id == tab_id {
            entry.tab_button.add_css_class("chostty-tab-active");
        } else {
            entry.tab_button.remove_css_class("chostty-tab-active");
        }
...
#[cfg(feature = "webkit")]
const CHOSTTY_BROWSER_EDITABLE_STATE_HANDLER: &str = "chosttyEditableState";

#[cfg(feature = "webkit")]
const CHOSTTY_BROWSER_EDITABLE_STATE_SCRIPT: &str = r#"
(() => {
  const handler = globalThis.webkit?.messageHandlers?.chosttyEditableState;
...
fn create_browser_widget(
    ...
    let _ = user_content_manager
        .register_script_message_handler(CHOSTTY_BROWSER_EDITABLE_STATE_HANDLER, None);
    user_content_manager.add_script(&webkit6::UserScript::new(
        CHOSTTY_BROWSER_EDITABLE_STATE_SCRIPT,
    ...
    ));
    {
        ...
        user_content_manager.connect_script_message_received(
            Some(CHOSTTY_BROWSER_EDITABLE_STATE_HANDLER),
            move |_, value| {
    ...
    nav_bar.add_css_class("chostty-pane-header");
    ...
    vbox.add_css_class("chostty-browser");
...
mod tests {
    ...
    fn pane_css_keeps_entry_layout_classes_separate_from_shared_theme() {
        assert!(PANE_CSS.contains(".chostty-tab-rename-entry"));
        assert!(PANE_CSS.contains(".chostty-browser-url-entry"));
        assert!(PANE_CSS.contains(".chostty-browser-search-entry"));
