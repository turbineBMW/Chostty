...
const SPLIT_RATIO_STATE_KEY: &str = "chostty-split-ratio-state";
const EMPTY_WORKSPACE_PAGE_NAME: &str = "chostty-empty-workspaces";
...
fn save_session_now(state: &State) {
    ...
    if let Err(err) = layout_state::save_session_atomic(&session) {
        eprintln!("chostty: failed to save session state: {err}");
    }
}
...
fn build_sidebar_menu_item(
    ...
    let icon = gtk::Image::builder()
        ...
        .pixel_size(16)
        .build();
    icon.add_css_class("chostty-sidebar-menu-item-icon");
    ...
    let text = gtk::Label::builder()
        ...
        .hexpand(true)
        .build();
    text.add_css_class("chostty-sidebar-menu-item-label");
    ...
    if let Some(shortcut_label) = shortcut_label {
        let shortcut = gtk::Label::builder()
            ...
            .build();
        shortcut.add_css_class("chostty-sidebar-menu-item-shortcut");
    ...
    button.add_css_class("chostty-sidebar-menu-item");
    button
}
            ...
            Rc::new(
                move |previous: &app_config::AppConfig, updated: &app_config::AppConfig| {
                    ...
                    if let Err(err) = app_config::save(updated) {
                        ...
                        let detail = format!("Failed to save Chostty settings: {err}");
                        eprintln!("chostty: {detail}");
                        show_runtime_error(
                            &state_for_config_changed,
...
fn build_sidebar_menu_popover(state: &State) -> gtk::Popover {
    ...
    popover.add_css_class("chostty-menu-popover");
    ...
    let menu_box = gtk::Box::builder()
        ...
        .spacing(0)
        .build();
    menu_box.add_css_class("chostty-sidebar-menu");
...
const HOST_ENTRY_CSS_CLASS: &str = "chostty-host-entry";
const WORKSPACE_RENAME_ENTRY_CSS_CLASS: &str = "chostty-ws-rename-entry";
const WORKSPACE_RENAME_ENTRY_CSS_CLASSES: [&str; 2] =
    [HOST_ENTRY_CSS_CLASS, WORKSPACE_RENAME_ENTRY_CSS_CLASS];

const BASE_CSS: &str = r#"
:root {
    --chostty-host-entry-bg: rgba(255, 255, 255, 0.98);
    --chostty-host-entry-fg: rgba(15, 23, 42, 0.96);
    --chostty-host-entry-border: rgba(15, 23, 42, 0.16);
    --chostty-host-entry-border-focus: rgba(0, 145, 255, 0.72);
    --chostty-host-entry-placeholder: rgba(15, 23, 42, 0.5);
}
@media (prefers-color-scheme: dark) {
    :root {
        --chostty-host-entry-bg: rgba(44, 44, 48, 0.98);
        --chostty-host-entry-fg: rgba(255, 255, 255, 0.96);
        --chostty-host-entry-border: rgba(255, 255, 255, 0.14);
        --chostty-host-entry-border-focus: rgba(0, 145, 255, 0.78);
        --chostty-host-entry-placeholder: rgba(255, 255, 255, 0.48);
    }
}
.chostty-host-entry {
    background-color: var(--chostty-host-entry-bg);
    color: var(--chostty-host-entry-fg);
    border: 1px solid var(--chostty-host-entry-border);
    border-radius: 6px;
    caret-color: currentColor;
}
.chostty-host-entry:focus-within {
    border-color: var(--chostty-host-entry-border-focus);
}
.chostty-host-entry text {
    background-color: transparent;
    color: var(--chostty-host-entry-fg);
}
.chostty-host-entry text placeholder {
    color: var(--chostty-host-entry-placeholder);
}
.chostty-host-entry image {
    color: var(--chostty-host-entry-placeholder);
}
@define-color chostty_divider_color color-mix(
    in srgb,
    @window_fg_color 8%,
    @window_bg_color
);
...
.chostty-window paned > separator {
    background-color: transparent;
    min-width: 0;
    min-height: 0;
    padding: 0;
...
.chostty-window paned.chostty-split-paned > separator {
    min-width: 0;
    min-height: 0;
    padding: 0;
    margin: 0;
...
.chostty-window paned.chostty-split-paned.horizontal > separator {
    border-right: 1px solid @chostty_divider_color;
}
.chostty-window paned.chostty-split-paned.vertical > separator {
    border-bottom: 1px solid @chostty_divider_color;
}
.chostty-sidebar {
...
.chostty-ws-star-btn:hover {
    color: alpha(@window_fg_color, 0.9);
}
row:selected .chostty-ws-star-btn {
    color: alpha(@window_fg_color, 0.85);
}
.chostty-ws-star-btn-active {
    color: @accent_bg_color;
}
.chostty-ws-rename-entry {
    min-height: 0;
    padding: 0 4px;
    margin: 0;
}
...
.chostty-notify-msg-unread {
    color: alpha(@accent_bg_color, 0.9);
    font-size: 11px;
}
.chostty-sidebar-row-unread {
...
.chostty-sidebar-row-unread .chostty-ws-name {
    color: @window_fg_color;
    font-weight: 700;
}
.chostty-drop-above .chostty-sidebar-row-box {
    border-radius: 0;
    box-shadow: 0 -2px 0 0 @accent_bg_color;
}
.chostty-drop-below .chostty-sidebar-row-box {
    border-radius: 0;
    box-shadow: 0 2px 0 0 @accent_bg_color;
}
.chostty-tab-drop-target {
    background-color: alpha(@accent_bg_color, 0.18);
    border-radius: 8px;
}
.chostty-sidebar row:drop(active) {
    box-shadow: none;
}
.chostty-sidebar-title {
    color: alpha(@window_fg_color, 0.92);
    font-size: 14px;
    font-weight: 700;
    letter-spacing: 0.2px;
}
.chostty-sidebar-tool-btn {
...
.chostty-sidebar-tool-btn:hover {
    background: alpha(@window_fg_color, 0.08);
    color: @window_fg_color;
}
.chostty-sidebar-search {
    margin: 0 8px 6px 8px;
}
popover.chostty-menu-popover > contents {
    padding: 4px;
    border-radius: 16px;
...
.chostty-sidebar-menu {
    margin: 2px;
    min-width: 250px;
}
.chostty-sidebar-menu-section-title {
    color: alpha(@window_fg_color, 0.5);
    font-size: 11px;
    font-weight: 700;
    letter-spacing: 0.8px;
}
.chostty-sidebar-menu-item {
    padding: 0;
    min-height: 0;
    border-radius: 12px;
}
.chostty-sidebar-menu-item > box {
    padding: 9px 12px;
    border-radius: 12px;
    transition: background 160ms ease;
}
.chostty-sidebar-menu-item:hover > box,
.chostty-sidebar-menu-item:active > box,
.chostty-sidebar-menu-item:checked > box {
    background: alpha(@window_fg_color, 0.1);
}
.chostty-sidebar-menu-item-label {
    color: @window_fg_color;
    font-size: 13px;
    font-weight: 500;
}
.chostty-sidebar-menu-item-shortcut {
    color: alpha(@window_fg_color, 0.5);
    font-size: 12px;
    font-weight: 500;
}
.chostty-sidebar-menu-item-icon {
...
.chostty-sidebar-btn.chostty-tab-drop-target {
...
.chostty-empty-layout {
    min-width: 360px;
}
.chostty-empty-icon {
    color: alpha(@window_fg_color, 0.18);
}
.chostty-empty-kicker {
...
pub fn build_window(app: &adw::Application) {
    ...
    for path in [
        exe_dir
            .as_ref()
            .map(|d| d.join("../../rust/chostty-host-linux/icons")),
    ...
    let title = format!("Chostty v{}", crate::VERSION);
    let window = adw::ApplicationWindow::builder()
        ...
        .default_height(900)
        .build();
    window.add_css_class("chostty-window");

    // On Wayland compositors with xdg-decoration support, the compositor
    // already provides the window chrome, so keep Chostty from rendering a
    ...
    stack.add_css_class("chostty-content");
    ...
    let sidebar_search_entry = gtk::SearchEntry::builder()
        ...
        .hexpand(true)
        .build();
    sidebar_search_entry.add_css_class("chostty-sidebar-search");
    ...
    search_btn.add_css_class("chostty-sidebar-tool-btn");
    ...
    let sidebar_title_label = gtk::Label::builder()
        ...
        .build();
    sidebar_title_label.add_css_class("chostty-sidebar-title");
    ...
    menu_btn.add_css_class("chostty-sidebar-tool-btn");
    ...
    let new_ws_btn = gtk::Button::builder()
        ...
        .margin_bottom(6)
        .build();
    new_ws_btn.add_css_class("chostty-sidebar-btn");
    ...
    btn_drop.set_preload(true);
    {
        ...
        btn_drop.connect_motion(move |_, _, _| {
            if pane::is_tab_dragging() {
                btn.add_css_class("chostty-tab-drop-target");
            } else {
                btn.add_css_class("chostty-sidebar-btn-trash-hover");
            }
    ...
    }
    {
        ...
        btn_drop.connect_leave(move |_| {
            btn.remove_css_class("chostty-sidebar-btn-trash-hover");
            btn.remove_css_class("chostty-tab-drop-target");
        });
    }
    ...
    let sidebar = gtk::Box::builder()
        ...
        .width_request(220)
        .build();
    sidebar.add_css_class("chostty-sidebar");
    ...
    let main_paned = gtk::Paned::builder()
        ...
        .end_child(&stack)
        .build();
    main_paned.add_css_class("chostty-sidebar-paned");
...

    {
        ...
        pane::on_tab_drag_change(move |dragging| {
            if dragging {
                btn.add_css_class("chostty-tab-drag-active");
            } else {
                btn.remove_css_class("chostty-tab-drag-active");
                btn.remove_css_class("chostty-tab-drop-target");
            }
        });
    }

    {
        ...
        btn_drop.connect_drop(move |_, value, _, _| {
            ...
            btn.remove_css_class("chostty-sidebar-btn-trash");
            btn.remove_css_class("chostty-sidebar-btn-trash-hover");
            btn.remove_css_class("chostty-tab-drop-target");
...
fn build_sidebar_row(
    ...
    notify_dot.add_css_class("chostty-notify-dot-hidden");
    ...
    let name_label = gtk::Label::builder()
        ...
        .build();
    name_label.add_css_class("chostty-ws-name");

    let favorite_button = gtk::Button::with_label("\u{2606}");
    favorite_button.add_css_class("flat");
    favorite_button.add_css_class("chostty-ws-star-btn");
    ...
    let path_label = gtk::Label::builder()
        ...
        .margin_start(8)
        .build();
    path_label.add_css_class("chostty-ws-path");
    ...
    let vbox = gtk::Box::builder()
        ...
        .spacing(2)
        .build();
    vbox.add_css_class("chostty-sidebar-row-box");
    vbox.append(&top_row);
...
fn build_empty_workspace_page(state: &State) -> gtk::Widget {
    let icon = gtk::Image::builder()
        ...
        .halign(gtk::Align::Center)
        .build();
    icon.add_css_class("chostty-empty-icon");
    ...
    let kicker = gtk::Label::builder()
        ...
        .halign(gtk::Align::Center)
        .build();
    kicker.add_css_class("chostty-empty-kicker");
    ...
    let title = gtk::Label::builder()
        ...
        .wrap(true)
        .build();
    title.add_css_class("chostty-empty-title");
    ...
    let copy = gtk::Label::builder()
        ...
        .wrap(true)
        .build();
    copy.add_css_class("chostty-empty-copy");
    ...
    let hint = gtk::Label::builder()
        ...
        .wrap(true)
        .build();
    hint.add_css_class("chostty-empty-hint");
    ...
    let layout = gtk::Box::builder()
        ...
        .build();
    layout.add_css_class("chostty-empty-layout");
    layout.append(&icon);
    ...
    let page = gtk::Box::builder()
        ...
        .vexpand(true)
        .build();
    page.add_css_class("chostty-empty-state");
    page.append(&layout);
...
fn set_workspace_favorite_visual(workspace: &Workspace) {
    ...
    if workspace.favorite {
        workspace
            .favorite_button
            .add_css_class("chostty-ws-star-btn-active");
    } else {
        workspace
            .favorite_button
            .remove_css_class("chostty-ws-star-btn-active");
    }
}
...
fn install_workspace_row_interactions(
    ...
    }
    {
        ...
        drag_source.connect_drag_begin(move |source, _| {
            ...
            s.new_ws_btn.add_css_class("chostty-sidebar-btn-trash");
            drop(s);
    ...
    }
    {
        ...
        drag_source.connect_drag_end(move |_, _, _| {
            ...
            s.new_ws_btn.remove_css_class("chostty-sidebar-btn-trash");
            s.new_ws_btn
                .remove_css_class("chostty-sidebar-btn-trash-hover");
    ...
    let drop_handled = Rc::new(Cell::new(false));
    {
        ...
        drop_target.connect_motion(move |_, _x, y| {
            ...
            r.remove_css_class("chostty-drop-above");
            r.remove_css_class("chostty-drop-below");
            r.remove_css_class("chostty-tab-drop-target");
            ...
            match dragged_workspace {
                Some(ref dragged_workspace_id) if dragged_workspace_id != &target_workspace_id => {
                    if y < h / 2.0 {
                        r.add_css_class("chostty-drop-above");
                    } else {
                        r.add_css_class("chostty-drop-below");
                    }
                }
                None => {
                    r.add_css_class("chostty-tab-drop-target");
                }
                _ => {}
            }
...
fn open_workspace_by_path_dialog(state: &State) {
    let Some(initial_directory) = effective_workspace_start_directory(state) else {
        show_runtime_error(
            ...
            "Chostty could not determine a default starting directory.",
        );
        return;
    };
...
fn switch_workspace(state: &State, idx: usize) {
    ...
    if let Some((notify_dot, path_label, sidebar_row, folder_path)) = unread_handles {
        notify_dot.remove_css_class("chostty-notify-dot");
        notify_dot.add_css_class("chostty-notify-dot-hidden");
        // Restore the path label from notification message back to folder path
        path_label.remove_css_class("chostty-notify-msg-unread");
        path_label.add_css_class("chostty-ws-path");
        ...
        if let Some(row_box) = sidebar_row.child() {
            row_box.remove_css_class("chostty-sidebar-row-unread");
        }
    }
...
/// Find the focused pane widget (a gtk::Box with class chostty-pane-toolbar child)
    ...
    while let Some(w) = widget {
        if let Some(bx) = w.downcast_ref::<gtk::Box>() {
            ...
            while let Some(c) = child {
                if c.has_css_class("chostty-pane-header") {
                    return Some((ws_id, w));
                }
...
fn spawn_new_instance(state: &State) -> bool {
    let exe = match std::env::current_exe() {
        Ok(exe) => exe,
        Err(err) => {
            let detail = format!("Failed to resolve the current Chostty executable: {err}");
            eprintln!("chostty: {detail}");
            show_runtime_error(state, "Failed to open a new Chostty instance", &detail);
            return false;
        }
    };
    ...
    match std::process::Command::new(exe).spawn() {
        Ok(_) => true,
        Err(err) => {
            let detail = format!("Failed to launch a new Chostty instance: {err}");
            eprintln!("chostty: {detail}");
            show_runtime_error(state, "Failed to open a new Chostty instance", &detail);
            false
        }
...
fn mark_workspace_unread_with_message(state: &State, ws_id: &str, message: &str) {
    let sound = {
        ...
        if idx != active_idx {
            ws.unread = true;
            ws.notify_dot.remove_css_class("chostty-notify-dot-hidden");
            ws.notify_dot.add_css_class("chostty-notify-dot");
            // Replace the path label text with the notification message
            ws.path_label.set_label(message);
            ws.path_label.remove_css_class("chostty-ws-path");
            ws.path_label.add_css_class("chostty-notify-msg-unread");
            ws.path_label.set_visible(true);
            // Add glow pulse to the sidebar row box
            if let Some(row_box) = ws.sidebar_row.child() {
                row_box.add_css_class("chostty-sidebar-row-unread");
            }
...
mod tests {
    ...
    fn build_window_css_preserves_transparent_content_background() {
        ...
        assert!(css.contains(".chostty-host-entry"));
        assert!(css.contains(".chostty-host-entry text"));
        assert!(css.contains(".chostty-host-entry text placeholder"));
        assert!(css.contains(".chostty-window paned > separator"));
        assert!(css.contains("min-width: 0;"));
        assert!(css.contains("paned.chostty-split-paned > separator"));
        assert!(css.contains("paned.chostty-split-paned.horizontal > separator"));
        assert!(css.contains("paned.chostty-split-paned.vertical > separator"));
        assert!(css.contains(".chostty-content"));
        assert!(css.contains(".chostty-content {\n    background-color: transparent;"));
    }
    ...
    fn base_css_defines_theme_aware_host_entry_styles() {
        ...
        assert!(BASE_CSS.contains(".chostty-host-entry"));
        assert!(BASE_CSS.contains(".chostty-host-entry text"));
        assert!(BASE_CSS.contains(".chostty-host-entry text placeholder"));
    ...
    fn workspace_rename_entry_uses_shared_host_entry_class() {
        ...
        assert!(BASE_CSS.contains(".chostty-ws-rename-entry"));
    }
