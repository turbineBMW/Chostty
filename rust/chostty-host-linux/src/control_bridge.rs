//! Bridge the chostty control socket onto the GTK host state.
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let listener = match UnixListener::bind(&path) {
                ...
                Err(error) => {
                    eprintln!(
                        "chostty: control socket bind failed ({}): {error}",
                        path.display()
                    );
                    return;
                }
            ...
            eprintln!("chostty: control socket at {}", path.display());
            ...
            for stream in listener.incoming() {
                match stream {
                    Ok(stream) => {
                        ...
                        std::thread::Builder::new()
                            .name("chostty-ctrl-conn".into())
                            .spawn(move || {
                                if let Err(error) = handle_client(stream, dispatch.as_ref()) {
                                    eprintln!("chostty: control connection error: {error}");
                                }
                            })
                            .ok();
                    }
                    Err(error) => {
                        eprintln!("chostty: control accept error: {error}");
                    }
                }
            }
        })
