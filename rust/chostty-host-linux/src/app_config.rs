...
fn backup_invalid_existing_config_with_detail(path: &Path, detail: &str) -> Result<(), String> {
    ...
    eprintln!(
        "chostty: {detail}; backed up `{}` to `{}` before rewriting settings",
        path.display(),
        backup_path.display()
    );
    Ok(())
...
mod tests {
    ...
    #[test]
    fn settings_path_in_uses_chostty_settings_json() {
        let path = settings_path_in(Path::new("/tmp/example"));

        assert_eq!(path, Path::new("/tmp/example/chostty/settings.json"));
    }
