...
pub const CONFIG_DIR_NAME: &str = "chostty";
...
mod tests {
    ...
    #[test]
    fn config_dir_path_in_uses_chostty_config_dir() {
        let base = Path::new("/tmp/example");
        assert_eq!(
            config_dir_path_in(base),
            PathBuf::from("/tmp/example/chostty")
        );
    }

    #[test]
    fn shortcuts_path_in_uses_chostty_shortcuts_json() {
        let base = Path::new("/tmp/example");
        assert_eq!(
            shortcuts_path_in(base),
            PathBuf::from("/tmp/example/chostty/shortcuts.json")
        );
    }
